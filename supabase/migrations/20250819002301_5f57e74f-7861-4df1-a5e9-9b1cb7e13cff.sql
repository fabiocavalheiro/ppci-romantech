...
INSERT INTO public.locations (name, address, description, client_id) VALUES 
  ('Matriz - São Paulo', 'Av. Paulista, 1000 - São Paulo, SP', 'Sede principal da empresa', (SELECT id FROM public.clients LIMIT 1)),
  ('Filial - Rio de Janeiro', 'Av. Copacabana, 500 - Rio de Janeiro, RJ', 'Filial regional Rio de Janeiro', (SELECT id FROM public.clients LIMIT 1)),
  ('Centro de Distribuição', 'Rodovia Anhanguera, km 25 - Jundiaí, SP', 'Centro de distribuição e logística', (SELECT id FROM public.clients LIMIT 1))
